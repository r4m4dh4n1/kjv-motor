-- MANUAL SETUP: Retroactive System
...
-- 11. Create triggers
DROP TRIGGER IF EXISTS update_retroactive_operational_updated_at ON public.retroactive_operational;
CREATE TRIGGER update_retroactive_operational_updated_at 
    BEFORE UPDATE ON public.retroactive_operational 
    FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS update_monthly_adjustments_updated_at ON public.monthly_adjustments;
...
-- SELECT 'retroactive_operational table created' as status WHERE EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'retroactive_operational');
-- SELECT 'monthly_adjustments table created' as status WHERE EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'monthly_adjustments');
-- SELECT 'operational table has retroactive columns' as status WHERE EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'operational' AND column_name = 'is_retroactive');