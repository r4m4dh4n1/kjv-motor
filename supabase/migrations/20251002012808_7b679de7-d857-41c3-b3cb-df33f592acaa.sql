-- Create close_month_by_division function for per-division month closing
...
BEGIN
    ...
    INSERT INTO pembukuan_history (
        ...
        divisi, created_at, updated_at, closed_month, closed_year
    )
    SELECT 
        id, tanggal, keterangan, debit, kredit, company_id, cabang_id,
        divisi, created_at, updated_at, target_month, target_year
    FROM pembukuan 
    WHERE EXTRACT(MONTH FROM tanggal) = target_month 
    AND EXTRACT(YEAR FROM tanggal) = target_year
    AND divisi = target_division;
