-- Update close_month function to follow correct business logic
-- Only move completed cicilan to history, keep pending ones active
-- Keep ready pembelian and booked penjualan active (already implemented correctly)
...
BEGIN
  ...
  -- Move ONLY COMPLETED cicilan to history FIRST (they reference penjualans)
  -- Leave pending/unfinished cicilan active so they carry over to next month
  ...
  WHERE EXTRACT(MONTH FROM tanggal_bayar) = target_month 
    ...
    AND status = 'completed';  -- Only move completed cicilan
  
  GET DIAGNOSTICS cicilan_count = ROW_COUNT;

  -- Delete ONLY completed cicilan (pending ones remain active)
  DELETE FROM cicilan 
  WHERE EXTRACT(MONTH FROM tanggal_bayar) = target_month 
    AND EXTRACT(YEAR FROM tanggal_bayar) = target_year
  ...
  -- EXCLUDE penjualans with status 'booked' - they should remain active and carry over
  INSERT INTO penjualans_history (
    ...
    created_at, updated_at, closed_month, closed_year, subsidi_ongkir
  )
  SELECT 
    id, tanggal, cabang_id, brand_id, jenis_id, tahun, kilometer, pajak,
    harga_beli, harga_jual, harga_bayar, keuntungan, sisa_bayar, titip_ongkir,
    ...
    biaya_lain_lain, catatan, keterangan_biaya_lain, reason_update_harga,
    divisi, tt, warna, plat, status, jenis_pembayaran, sisa_ongkir,
    created_at, updated_at, target_month, target_year, subsidi_ongkir
  FROM penjualans 
  WHERE EXTRACT(MONTH FROM tanggal) = target_month 
    AND EXTRACT(YEAR FROM tanggal) = target_year
    AND status != 'booked';  -- Keep booked penjualans active
  
  GET DIAGNOSTICS penjualan_count = ROW_COUNT;

  -- Delete penjualans EXCEPT those with status 'booked' (they carry over to next month)
  ...
  -- EXCLUDE pembelian with status 'ready' - they should remain active and carry over
  ...
  WHERE EXTRACT(MONTH FROM tanggal_pembelian) = target_month 
    ...
    AND status != 'ready';  -- Keep ready pembelian active
  
  GET DIAGNOSTICS pembelian_count = ROW_COUNT;

  -- Delete pembelian EXCEPT those with status 'ready' (they carry over to next month)
