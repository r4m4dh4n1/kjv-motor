-- Add tanggal_pencatatan column to biro_jasa table
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'biro_jasa' AND column_name = 'tanggal_pencatatan'
    ) THEN
        ALTER TABLE public.biro_jasa ADD COLUMN tanggal_pencatatan DATE;
        RAISE NOTICE 'Added tanggal_pencatatan column to biro_jasa table';
    ELSE
        RAISE NOTICE 'tanggal_pencatatan column already exists in biro_jasa table';
    END IF;
END $$;

-- Add tanggal_pencatatan column to biro_jasa_history table
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'biro_jasa_history' AND column_name = 'tanggal_pencatatan'
    ) THEN
        ALTER TABLE public.biro_jasa_history ADD COLUMN tanggal_pencatatan DATE;
        RAISE NOTICE 'Added tanggal_pencatatan column to biro_jasa_history table';
    ELSE
        RAISE NOTICE 'tanggal_pencatatan column already exists in biro_jasa_history table';
    END IF;
END $$;

-- Backfill existing 'Selesai' records with 'tanggal' (entry date) as 'tanggal_pencatatan'
-- This ensures historical reports don't lose data
UPDATE public.biro_jasa 
SET tanggal_pencatatan = tanggal 
WHERE status = 'Selesai' AND tanggal_pencatatan IS NULL;

UPDATE public.biro_jasa_history 
SET tanggal_pencatatan = tanggal 
WHERE status = 'Selesai' AND tanggal_pencatatan IS NULL;

-- Update close_month function to include tanggal_pencatatan
...
BEGIN
    ...
    -- Move biro_jasa to history (UPDATED with tanggal_pencatatan)
    INSERT INTO biro_jasa_history (
        ...
        created_at, updated_at, divisi, tanggal_pencatatan
    )
    SELECT 
        id, tanggal, jenis_pengurusan, jenis_motor, tahun, warna, plat_nomor,
        estimasi_biaya, estimasi_tanggal_selesai, dp, sisa, biaya_modal,
        keuntungan, total_bayar, status, rekening_tujuan_id, keterangan,
        created_at, updated_at, COALESCE(divisi, 'unknown'), tanggal_pencatatan
    FROM biro_jasa 
    WHERE EXTRACT(MONTH FROM tanggal) = target_month 
    AND EXTRACT(YEAR FROM tanggal) = target_year
    AND (target_division IS NULL OR divisi = target_division);
