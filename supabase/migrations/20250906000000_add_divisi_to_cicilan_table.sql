-- Add divisi column to cicilan table to fix "column divisi does not exist" error
-- This addresses the root cause where cicilan table lacks the divisi column

-- Add divisi column to cicilan table
ALTER TABLE public.cicilan 
ADD COLUMN IF NOT EXISTS divisi VARCHAR(50) DEFAULT 'unknown';

-- Update existing cicilan records with divisi from penjualans table
...
-- Update the close_month function to handle divisi properly
CREATE OR REPLACE FUNCTION close_month(target_month integer, target_year integer, target_division text DEFAULT NULL)
RETURNS json
LANGUAGE plpgsql
AS $$
DECLARE
