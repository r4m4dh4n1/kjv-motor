-- Add missing divisi fields to history tables
-- This script fixes the "field divisi not found" error in close_month function

-- Add divisi field to cicilan_history table
ALTER TABLE public.cicilan_history 
ADD COLUMN IF NOT EXISTS divisi VARCHAR(50);

-- Add divisi field to biro_jasa_history table  
ALTER TABLE public.biro_jasa_history 
ADD COLUMN IF NOT EXISTS divisi VARCHAR(50);

-- Add divisi field to assets_history table
ALTER TABLE public.assets_history 
ADD COLUMN IF NOT EXISTS divisi VARCHAR(50);

-- Drop existing close_month function first to avoid parameter name conflicts
DROP FUNCTION IF EXISTS close_month(INTEGER, INTEGER, TEXT);

-- Update the close_month function to handle divisi fields correctly
CREATE OR REPLACE FUNCTION close_month(p_month INTEGER, p_year INTEGER, p_notes TEXT DEFAULT NULL)
RETURNS JSON AS $$
DECLARE
...
BEGIN
    ...
    IF EXISTS (
        ...
        WHERE closure_month = p_month AND closure_year = p_year
    ) THEN
        RAISE EXCEPTION 'Month % of year % is already closed', p_month, p_year;
    END IF;

    -- Move cicilan records
    ...
    RETURN json_build_object(
        ...
        'total_records', cicilan_count + fee_count + pembukuan_count + penjualan_count + pembelian_count + operational_count + biro_jasa_count + assets_count,
        'closure_date', NOW(),
        'notes', p_notes
    );
END;
