-- RESTORE BIRO_JASA AND ASSETS HANDLING TO CLOSE_MONTH FUNCTION
-- This migration fixes the issue where the latest close_month function
-- (20250907000000_final_divisi_cicilan_fix.sql) removed biro_jasa and assets handling

-- Drop the current close_month function
DROP FUNCTION IF EXISTS public.close_month(integer, integer, text);

-- Create the complete close_month function with ALL tables including biro_jasa and assets
...
-- Verification: Show current function signature
SELECT 
...
-- Verification: Check that all required divisi columns exist
DO $$
BEGIN
    -- Check biro_jasa
    IF NOT EXISTS (
...
DO $$
BEGIN
    RAISE NOTICE 'RESTORATION COMPLETE: close_month function now handles ALL tables: penjualans, cicilan, pembukuan, biro_jasa, and assets with proper divisi filtering!';
END $$;