-- PRODUCTION FIX: Fix close_month function schema mismatch
-- Run this in Supabase SQL Editor

-- Drop existing function first
DROP FUNCTION IF EXISTS close_month(integer, integer, text);

-- Recreate function with correct column names for monthly_closures table
...
BEGIN
    ...
    INSERT INTO biro_jasa_history (
        ...
        rekening_tujuan_id, keterangan, created_at, updated_at, 
        closed_month, closed_year, closed_at
    )
    SELECT 
        id, tanggal, jenis_pengurusan, brand_id, jenis_motor_id, jenis_motor,
    ...
    -- FIXED: Use closure_date instead of created_at (which doesn't exist in monthly_closures table)
