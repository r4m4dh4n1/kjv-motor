-- Fix restore_month function to handle ID conflicts properly
...
BEGIN
  ...
  -- Restore assets from history (assets table uses auto-increment ID)
  INSERT INTO assets (
  ...
  -- Restore biro_jasa from history (biro_jasa table uses auto-increment ID)
  ...
  -- Restore operational from history (operational uses UUID, can keep original ID)
  ...
  -- Restore pembelian from history (pembelian uses manual ID, can keep original)
  ...
  -- Restore pembukuan from history (pembukuan uses auto-increment ID)
  ...
  -- Restore penjualans from history (penjualans uses manual ID, can keep original)
  ...
  -- Restore fee_penjualan from history (fee_penjualan uses auto-increment ID)
  ...
  -- Restore cicilan from history (cicilan uses manual ID, can keep original)
  INSERT INTO cicilan (
