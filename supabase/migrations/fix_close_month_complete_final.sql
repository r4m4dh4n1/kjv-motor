-- Complete fix for close_month function
-- This script creates a comprehensive close_month function that handles all tables properly

-- Drop existing function first
DROP FUNCTION IF EXISTS public.close_month(integer, integer, text);

-- Create the complete close_month function
...
BEGIN
    ...
    -- Move biro_jasa to history (FIXED: proper column mapping)
    ...
    SELECT 
        ...
        tahun, jenis_pengurusan, keterangan, estimasi_biaya, estimasi_tanggal_selesai,
        dp, sisa, rekening_tujuan_id, status, created_at, updated_at,
        target_month, target_year, COALESCE('unknown', 'unknown')
    FROM biro_jasa 
    WHERE EXTRACT(MONTH FROM tanggal) = target_month 
    AND EXTRACT(YEAR FROM tanggal) = target_year;
    ...
    SELECT 
        id, jenis_asset, tanggal_perolehan, harga_asset, keterangan,
        created_at, updated_at, target_month, target_year, COALESCE('unknown', 'unknown')
    FROM assets 
