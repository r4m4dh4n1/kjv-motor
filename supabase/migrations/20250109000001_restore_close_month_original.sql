-- RESTORE CLOSE_MONTH FUNCTION TO ORIGINAL VERSION (WITHOUT DIVISI)
-- This migration restores the close_month function to its original signature and logic

-- Drop the current close_month function with divisi parameter
DROP FUNCTION IF EXISTS public.close_month(integer, integer, text);

-- Create the ORIGINAL close_month function without divisi parameter
...
BEGIN
    ...
    -- Move penjualans to history (without divisi filtering)
    ...
    -- Move cicilan to history (without divisi filtering)
    ...
    -- Move pembukuan to history (without divisi filtering)
    ...
    -- Move biro_jasa to history (without divisi filtering)
    ...
    -- Move assets to history (without divisi filtering)
    ...
    -- Record the closure (without division field)
    INSERT INTO monthly_closures (closure_month, closure_year, closed_at, notes)
    VALUES (target_month, target_year, now(), notes);
...
-- Verification: Show the restored function signature
SELECT 
...
DO $$
BEGIN
    RAISE NOTICE 'RESTORE COMPLETE: close_month function restored to original signature: close_month(target_month integer, target_year integer, notes text DEFAULT NULL)';
END $$;