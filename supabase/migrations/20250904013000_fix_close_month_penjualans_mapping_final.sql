...
-- Recreate function with correct column mapping and monthly_closures column names
...
AND status = 'sold';
    ...
    -- STEP 6: Move remaining pembukuan entries (without pembelian_id)
    INSERT INTO pembukuan_history (
        id, tanggal, divisi, keterangan, debit, kredit, saldo, cabang_id,
        company_id, pembelian_id, created_at, updated_at, closed_month, closed_year, closed_at
    )
    ...
    -- STEP 7: Move operational from target month (FIXED COLUMNS)
INSERT INTO operational_history (
    id, tanggal, divisi, kategori, deskripsi, nominal, cabang_id,
...
-- STEP 8: Move biro_jasa from target month (FIXED COLUMNS - NO CLOSED COLUMNS)
...
SELECT 
    id, tanggal, jenis_pengurusan, NULL as brand_id, NULL as jenis_motor_id, jenis_motor,
    tahun, warna, plat_nomor, estimasi_biaya, estimasi_tanggal_selesai,
    dp, sisa, biaya_modal, keuntungan, total_bayar, status, rekening_tujuan_id,
    keterangan, created_at, updated_at
FROM biro_jasa 
...
AND EXTRACT(YEAR FROM tanggal) = target_year;
 ...
 -- STEP 9: Move assets from target month (FIXED COLUMNS)
INSERT INTO assets_history (
    id, jenis_asset, tanggal_perolehan, harga_asset, tanggal_jual, harga_jual,
    keuntungan, status, created_at, updated_at, closed_month, closed_year, closed_at
)
SELECT 
    id, jenis_asset, tanggal_perolehan, harga_asset, tanggal_jual, harga_jual,
    keuntungan, status, created_at, updated_at, target_month, target_year, now()
FROM assets 
...
AND EXTRACT(YEAR FROM tanggal_perolehan) = target_year;
    ...
    -- STEP 10: Move pencatatan_asset from target month (FIXED COLUMNS)
