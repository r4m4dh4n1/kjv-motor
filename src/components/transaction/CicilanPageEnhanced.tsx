...
const CicilanPageEnhanced = ({
  selectedDivision,
}: CicilanPageEnhancedProps) => {
  // State untuk data
  ...
  const getFilteredData = () => {
    ...
    if (selectedDivision !== "all") {
      filtered = filtered.filter(
        (item) => item.penjualans?.divisi === selectedDivision
      );
    }
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter((item) => {
        ...
        const jenisMotor =
          item.penjualans?.jenis_motor?.jenis_motor?.toLowerCase() || "";
        const keterangan = item.keterangan?.toLowerCase() || "";

        return (
          plat.includes(search) ||
    ...
    if (selectedCabang !== "all") {
      filtered = filtered.filter(
        (item) => item.penjualans?.cabang_id?.toString() === selectedCabang
      );
    }
    ...
    if (selectedJenisPembayaran !== "all") {
      filtered = filtered.filter(
        (item) => item.jenis_pembayaran === selectedJenisPembayaran
      );
    }

    // ✅ NEW LOGIC: Date filter hanya untuk status "selesai" atau "all"
    // Jika status = "pending", tampilkan SEMUA pending tanpa filter tanggal
    const shouldApplyDateFilter = selectedStatus !== "pending";

    // Filter tanggal (hanya jika bukan pending atau user set manual)
  ...
  const getCompaniesForPenjualan = () => {
    ...
    return companiesData.filter(
      (company) => company.divisi === selectedPenjualan.divisi
    );
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (jumlahBayar > selectedPenjualan.sisa_bayar && !overpaymentConfirmed) {
      ...
      const confirmed = window.confirm(
        `Pembayaran melebihi sisa bayar sebesar ${formatCurrency(
          overpayment
    ...
    try {
      ...
      const nextBatch =
        existingCicilan.length > 0 ? existingCicilan[0].batch_ke + 1 : 1;
      ...
      const { error: insertError } = await supabase.from("cicilan").insert([
        {
          ...
          tujuan_pembayaran_id: formData.tujuan_pembayaran_id
            ? parseInt(formData.tujuan_pembayaran_id)
            : null,
          status: sisaBayarBaru <= 0 ? "completed" : "pending",
          divisi: selectedDivision !== "all" ? selectedDivision : selectedPenjualan.divisi,
        },
      ]);
      ...
      if (sisaBayarBaru <= 0) {
        if (sisaBayarBaru < 0) {
          ...
          successMessage += ` Pembayaran telah lunas dengan kelebihan bayar ${formatCurrency(
            overpayment
          )}!`;
        } else {
  ...
  const handleDelete = async (cicilan: any) => {
    ...
    try {
      ...
      toast({
        title: "Berhasil",
        description:
          "Cicilan berhasil dihapus dan status penjualan dikembalikan ke 'booked'",
      });
  ...
  const handlePenjualanSelect = (penjualanId: string) => {
    const penjualan = penjualanData.find(
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  <Label htmlFor="tujuan_pembayaran">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                  <Label htmlFor="jenis-pembayaran-filter">
                    Jenis Pembayaran
                  </Label>
                  <Select
                    value={selectedJenisPembayaran}
                    onValueChange={setSelectedJenisPembayaran}
                  >
                    <SelectTrigger>
              ...
              <div className="flex items-center justify-between pt-2 border-t">
                ...
                <div className="text-sm text-gray-600">
                  Menampilkan {filteredData.length} dari {cicilanData.length}{" "}
                  data
                </div>
              </div>
            </CardContent>
                    ...
                    <TableBody>
                      {paginatedData.map((item, index) => (
                        <TableRow key={item.id}>
                          ...
                          <TableCell>
                            {item.penjualans?.brands?.name}{" "}
                            {item.penjualans?.jenis_motor?.jenis_motor}
                          </TableCell>
                          <TableCell>{item.penjualans?.plat}</TableCell>
                          <TableCell>
                            {item.penjualans?.cabang?.nama || "-"}
                          </TableCell>
                          <TableCell>
