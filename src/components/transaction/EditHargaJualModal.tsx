...
const EditHargaJualModal = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      // PERBAIKAN: Hitung total yang sudah dibayar dari DP + semua cicilan
      // Karena harga_bayar hanya menyimpan DP awal, kita perlu query total cicilan juga
      let totalSudahDibayar = penjualan.harga_bayar || 0;

      // Jika unit punya cicilan (cash_bertahap/kredit), hitung total dari tabel cicilan
      if (penjualan.jenis_pembayaran === 'cash_bertahap' || penjualan.jenis_pembayaran === 'kredit') {
        ...
        if (cicilanError) {
          console.error('Error fetching cicilan data:', cicilanError);
        } else if (cicilanData && cicilanData.length > 0) {
          const totalCicilan = cicilanData.reduce((sum: number, c: any) => sum + (c.jumlah_bayar || 0), 0);
          // Total sudah dibayar = DP awal (harga_bayar) + total cicilan
          totalSudahDibayar = (penjualan.harga_bayar || 0) + totalCicilan;
        }
      }

      const newSisaBayar = Math.max(0, hargaJualNum - totalSudahDibayar);

      // Update penjualan data - gunakan tabel 'penjualans' (dengan s)
      ...
      // PERBAIKAN: Update sisa_bayar di cicilan terakhir agar konsisten
      if (penjualan.jenis_pembayaran === 'cash_bertahap' || penjualan.jenis_pembayaran === 'kredit') {
        const { data: lastCicilan } = await supabase
          .from('cicilan')
          .select('id, sisa_bayar')
          .eq('penjualan_id', penjualan.id)
          .order('batch_ke', { ascending: false })
          .limit(1);

        if (lastCicilan && lastCicilan.length > 0) {
          await supabase
            .from('cicilan')
            .update({ sisa_bayar: newSisaBayar })
            .eq('id', lastCicilan[0].id);
        }
      }

      toast({
        title: "Berhasil",
        description: `Harga jual berhasil diupdate ke ${formatCurrency(hargaJualNum)}. Sisa bayar: ${formatCurrency(newSisaBayar)} (sudah dibayar: ${formatCurrency(totalSudahDibayar)})`,
        variant: "default"
      });
              ...
              <div className="space-y-1 text-sm">
                ...
                {penjualan?.sisa_bayar > 0 && (
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-orange-600">{formatCurrency(penjualan.sisa_bayar)}</span>
                  </div>
                )}
                {penjualan?.jenis_pembayaran && penjualan.jenis_pembayaran !== 'cash_penuh' && (
                  <div className="flex justify-between">
                    <span>Jenis Pembayaran:</span>
                    <span className="font-medium text-purple-600">
                      {penjualan.jenis_pembayaran === 'cash_bertahap' ? 'Cash Bertahap' : 'Kredit'}
                    </span>
                  </div>
                )}
