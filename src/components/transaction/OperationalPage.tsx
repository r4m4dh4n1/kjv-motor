...
const OperationalPage = ({ selectedDivision }: OperationalPageProps) => {
  ...
  const getTotalOperational = () => {
    // ✅ UPDATED: For division "start", exclude "kurang modal" categories
    // For other divisions, include all categories
    if (selectedDivision === "start") {
  ...
  // ✅ NEW: Calculate total OP Global (50% for display)
  const getTotalOPGlobal = () => {
    const totalOPGlobal = operationalData
      .filter((item) => isOPGlobalCategory(item.kategori))
      .reduce((total, item) => total + item.nominal, 0);
    // Return 50% of the total
    return totalOPGlobal / 2;
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`grid grid-cols-1 ${selectedDivision === "start" ? "md:grid-cols-3" : "md:grid-cols-4"} gap-6`}>
        <Card>
        ...
        {/* ✅ Conditional cards based on division */}
        {selectedDivision === "start" ? (
          // ✅ For START division: Show Total Biaya OP Global (50% only)
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600">
                    Total Biaya OP Global
                  </p>
                  <p className="text-2xl font-bold text-orange-600">
                    {formatCurrency(getTotalOPGlobal())}
                  </p>
                  <p className="text-xs text-gray-500 mt-1">
                    50% dari total OP Global
                  </p>
                </div>
          ...
          // ✅ For other divisions: Show original cards
          <>
            <Card>
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
