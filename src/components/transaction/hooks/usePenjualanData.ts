...
type DateFilterType = 'all' | 'last_month' | 'this_year' | 'custom' | 'today' | 'yesterday' | 'this_week' | 'last_week' | 'this_month' | 'last_year';
...
const getDateRange = (filter: DateFilterType): { start: string; end: string } | null => {
  const now = new Date();
...
const fetchPenjualanWithRelations = async (
  ...
  if (dateFilter && dateFilter !== 'all') {
    if (dateFilter === 'custom' && customDateRange) {
      query = query.gte('tanggal', customDateRange.start.toISOString().split('T')[0]);
      query = query.lte('tanggal', customDateRange.end.toISOString().split('T')[0]);
    } else {
  ...
  if (tableName === 'penjualans_history' && result.data && !result.error) {
    ...
    const enrichedData = result.data.map(item => ({
      ...
      cabang: { nama: `Cabang ID: ${item.cabang_id}` },
      brands: { name: `Brand ID: ${item.brand_id}` },
      jenis_motor: { jenis_motor: `Jenis ID: ${item.jenis_id}` },
      companies: { nama_perusahaan: `Company ID: ${item.company_id}` }
    }));
...
const fetchRelationsManually = async (historyData: any[]) => {
  ...
  try {
    // Ambil semua ID unik
    const cabangIds = [...new Set(historyData.map(item => item.cabang_id).filter(Boolean))];
    const brandIds = [...new Set(historyData.map(item => item.brand_id).filter(Boolean))];
    const jenisIds = [...new Set(historyData.map(item => item.jenis_id).filter(Boolean))];
    const companyIds = [...new Set(historyData.map(item => item.company_id).filter(Boolean))];
    
    // Fetch semua relasi sekaligus
    const [cabangResult, brandResult, jenisResult, companyResult] = await Promise.allSettled([
      cabangIds.length > 0 ? supabase.from('cabang').select('id, nama').in('id', cabangIds) : Promise.resolve({ data: [] }),
      ...
      companyIds.length > 0 ? supabase.from('companies').select('id, nama_perusahaan').in('id', companyIds) : Promise.resolve({ data: [] })
    ]);
    ...
    return historyData.map(item => ({
      ...
      companies: companyMap.get(item.company_id) || { nama_perusahaan: `Company ID: ${item.company_id}` }
    }));
    ...
    try {
      if (useCombined) {
        ...
        const [activeResult, historyResult] = await Promise.allSettled([
          fetchPenjualanWithRelations('penjualans', selectedDivision, dateFilter, customDateRange),
          fetchPenjualanWithRelations('penjualans_history', selectedDivision, dateFilter, customDateRange)
        ]);
        ...
        } else if (activeResult.status === 'fulfilled' && activeResult.value.error && 
                   historyResult.status === 'fulfilled' && historyResult.value.error) {
          setError(activeResult.value.error.message || historyResult.value.error.message);
        ...
        const result = await fetchPenjualanWithRelations('penjualans', selectedDivision, dateFilter, customDateRange);
