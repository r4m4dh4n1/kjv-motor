...
const BiroJasaPageEnhanced = ({
  ...
  const {
    ...
    fetchData,
    handleDelete,
  } = useBiroJasaData(selectedDivision);
  ...
  const getFilteredData = () => {
    const filtered = biroJasaData.filter((item) => {
      ...
      const matchesCabang =
        selectedCabang === "all" || item.cabang === selectedCabang;

      const matchesStatus =
        selectedStatus === "all" || item.status === selectedStatus;

      const matchesJenisPengurusan =
        selectedJenisPengurusan === "all" ||
      ...
      if (shouldApplyDateFilter && dateFilter !== "all") {
        ...
        switch (dateFilter) {
          ...
          case "tomorrow":
            ...
            matchesDate =
              itemDate >= startOfTomorrow && itemDate <= endOfTomorrow;
            break;
          case "yesterday":
            ...
            matchesDate =
              itemDate >= startOfYesterday && itemDate <= endOfYesterday;
            break;
          case "this_week":
          ...
          case "last_month":
            ...
            matchesDate =
              itemDate >= lastMonthStart && itemDate <= lastMonthEnd;
            break;
          case "this_year":
  ...
  const statistics = useMemo(() => {
    ...
    const dalamProses = filteredData.filter(
      (item) => item.status === "Dalam Proses" || item.status === "belum_lunas"
    ).length;

    const selesai = filteredData.filter(
      (item) => item.status === "Selesai" || item.status === "lunas"
    ).length;
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <Select
                    value={selectedJenisPengurusan}
                    onValueChange={setSelectedJenisPengurusan}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              <div className="flex gap-2">
                ...
                <div className="text-sm text-gray-500 flex items-center">
                  Menampilkan {filteredData.length} dari {biroJasaData.length}{" "}
                  data
                </div>
              </div>
            </CardContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">Data terfilter</p>
              </CardContent>
            </Card>
          </div>
