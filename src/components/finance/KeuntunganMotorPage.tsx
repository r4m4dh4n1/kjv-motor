...
const KeuntunganMotorPage = ({
  selectedDivision,
}: KeuntunganMotorPageProps) => {
  // State untuk data
  ...
  const [companiesModalDetail, setCompaniesModalDetail] = useState<
    CompanyModalDetail[]
  >([]);
  ...
  const totalHargaJual = keuntunganData.reduce(
    (sum, item) => sum + item.harga_jual,
    0
  );
  const totalKeuntungan = keuntunganData.reduce(
    (sum, item) => sum + item.profit,
    0
  );
  const roi =
    totalModal > 0 ? ((totalKeuntungan / totalModal) * 100).toFixed(2) : "0.00";

  // ← TAMBAHAN BARU: Rumus Cash Flow Operasional
  const cashFlowOperasional =
    totalModalCompanies +
  ...
  const shouldUseCombined =
    ["last_month", "this_year", "last_year"].includes(selectedPeriod) ||
  ...
  const fetchBiroJasaKeuntungan = async (
    dateRange: DateRange
  ): Promise<number> => {
  ...
  const fetchFromCombinedViews = async (
    ...
    try {
      ...
      let keuntunganQuery = supabase.from("penjualans_combined").select(`
      ...
      const [keuntunganResult, cabangResult, brandsResult, jenisMotorResult] =
        await Promise.all([
          keuntunganQuery,
      ...
      const jenisMotorMap = new Map(
        jenisMotorResult.data?.map((j) => [j.id, j]) || []
      );

      // Enrich data keuntungan dengan informasi relasi
      const enrichedKeuntungan =
        keuntunganResult.data?.map((item) => ({
          ...item,
      ...
      const cabangFilteredKeuntungan =
        selectedCabang === "all"
          ? filteredKeuntungan
          : filteredKeuntungan.filter(
              (item) => item.cabang_id.toString() === selectedCabang
            );
      ...
      if (selectedDivision !== "all") {
        bookedHargaBeliQuery = bookedHargaBeliQuery.eq(
          "divisi",
          selectedDivision
        );
      }
      ...
      let pembelianReadyQuery = supabase
        ...
        .select(
          "harga_beli, harga_final, divisi, tanggal_pembelian, id, cabang_id, status"
        );
      ...
      let pencatatanAssetQuery = supabase
        ...
        .or(
          "keterangan.ilike.%asset%,keterangan.ilike.%aset%,keterangan.ilike.%pengeluaran%"
        );

      if (selectedDivision !== "all") {
        pencatatanAssetQuery = pencatatanAssetQuery.eq(
          "divisi",
          selectedDivision
        );
      }
      ...
      const filteredBookedHargaBeli =
        bookedHargaBeliResult.data?.filter((item) => {
          const matchesCabang =
            selectedCabang === "all" ||
            item.cabang_id.toString() === selectedCabang;
          return matchesCabang;
      ...
      const filteredPembelianReady =
        pembelianReadyResult.data?.filter((item) => {
          const matchesCabang =
            selectedCabang === "all" ||
            item.cabang_id.toString() === selectedCabang;
          // Hapus filter tanggal, hanya filter cabang
      ...
      const filteredOperasional =
        operasionalResult.data?.filter((item) => {
          const itemDate = new Date(item.tanggal);
          const matchesDate = itemDate >= startDate && itemDate <= endDate;
          const matchesCabang =
            selectedCabang === "all" ||
      ...
      const filteredPencatatanAsset =
        pencatatanAssetResult.data?.filter((item) => {
          const itemDate = new Date(item.tanggal);
          const matchesDate = itemDate >= startDate && itemDate <= endDate;
          const matchesCabang =
            selectedCabang === "all" ||
      ...
      // Hitung totals
      const totalBooked = filteredBooked.reduce(
        (sum, item) => sum + (item.dp || 0),
        0
      );
      const totalBookedHargaBeli = filteredBookedHargaBeli.reduce(
        (sum, item) => sum + (item.harga_beli || 0),
        0
      );
      const totalPembelianReady = filteredPembelianReady.reduce(
        (sum, item) => sum + (item.harga_final || item.harga_beli || 0),
        0
      );
      const totalOperasional = filteredOperasional.reduce(
        (sum, item) => sum + (item.nominal || 0),
        0
      );
      const totalPencatatanAsset = filteredPencatatanAsset.reduce(
        (sum, item) => sum + (item.debit || 0),
        0
      );

      // Transform data keuntungan untuk display
      const keuntunganData: KeuntunganData[] = cabangFilteredKeuntungan.map(
        (item) => ({
          id: item.id,
          nama_motor: `${item.brands?.name || "Unknown"} ${
            item.jenis_motor?.jenis_motor || "Unknown"
          } ${item.tahun || ""} ${item.warna || ""}`.trim(),
      ...
      const totalProfitFiltered = keuntunganData.reduce(
        (sum, item) => sum + item.profit,
        0
      );
  ...
  const fetchFromActiveTables = async (
    dateRange: DateRange
  ): Promise<PeriodFilteredData> => {
    ...
    let keuntunganQuery = supabase.from("penjualans").select(`
        id,
        pembelian_id,
        harga_beli,
        harga_jual,
    ...
    const filteredKeuntungan =
      keuntunganResult.data?.filter((item) => {
        const itemDate = new Date(item.tanggal);
        const matchesDate = itemDate >= startDate && itemDate <= endDate;
        const matchesCabang =
          selectedCabang === "all" ||
  ...
  const fetchPeriodFilteredData = async (
    dateRange: DateRange
  ): Promise<PeriodFilteredData> => {
  ...
  const fetchAccumulativeData = async (
    dateRange: DateRange
    ...
    let unitYTDResult: any,
      pembelianYTDResult: any,
      penjualansBookedYTDResult: any,
      bookedYTDResult: any;
    ...
    if (shouldUseCombined) {
      ...
      if (selectedDivision !== "all")
        unitQuery = unitQuery.eq("divisi", selectedDivision);

      let pembelianQuery = supabase
        .from("pembelian_combined")
        .select(
          "harga_beli, harga_final, tanggal_pembelian, cabang_id, divisi, status"
        );
      if (selectedDivision !== "all")
      ...
      if (selectedDivision !== "all")
        penjualansBookedQuery = penjualansBookedQuery.eq(
          "divisi",
          selectedDivision
        );
    ...
    const filteredUnits = ((unitYTDResult.data as any[]) || []).filter(
      (item: any) => {
        const itemDate = new Date(item.tanggal);
        const matchesDate = itemDate >= startDate && itemDate <= endDate;
        const matchesCabang =
    ...
    const filteredPembelian = ((pembelianYTDResult.data as any[]) || []).filter(
      (item: any) => {
        const matchesCabang =
          selectedCabang === "all" ||
          item.cabang_id?.toString() === selectedCabang;
    ...
    const filteredPenjualansBooked = (
      (penjualansBookedYTDResult.data as any[]) || []
    ).filter((item: any) => {
      const matchesCabang =
        selectedCabang === "all" ||
        item.cabang_id?.toString() === selectedCabang;
    ...
    const filteredBooked = ((bookedYTDResult.data as any[]) || []).filter(
      (item: any) => {
        const matchesCabang =
          selectedCabang === "all" ||
          item.cabang_id?.toString() === selectedCabang;
    ...
    const totalPembelianFromPembelian = filteredPembelian.reduce(
      (sum: number, item: any) =>
        sum + (item.harga_final || item.harga_beli || 0),
      0
    );
    const totalPembelianFromPenjualans = filteredPenjualansBooked.reduce(
      (sum: number, item: any) => sum + (item.harga_beli || 0),
      0
    );
    const totalPembelianYTD =
      totalPembelianFromPembelian + totalPembelianFromPenjualans;
    const totalBookedYTD = filteredBooked.reduce(
      (sum: number, item: any) => sum + (item.dp || 0),
      0
    );
  ...
  const fetchCumulativeData = async (): Promise<CumulativeData> => {
    ...
    if (shouldUseCombined) {
      let modalQuery = supabase
        ...
        .or(
          "keterangan.ilike.%modal%,keterangan.ilike.%pemasukan%,keterangan.ilike.%setoran%"
        );
      if (selectedDivision !== "all")
    ...
    const filteredModal = ((modalResult.data as any[]) || []).filter(
      (item: any) => {
        const matchesCabang =
          selectedCabang === "all" ||
          item.cabang_id?.toString() === selectedCabang;
        const isModalIncome =
          item.keterangan?.toLowerCase().includes("modal") ||
    ...
    const totalModalPerusahaan = filteredModal.reduce(
      (sum: number, item: any) => sum + (item.kredit || 0),
      0
    );
  ...
  const fetchTotalModalCompanies = async (): Promise<number> => {
    ...
    const totalModal =
      companiesData?.reduce(
        (sum, company) => sum + Number(company.modal || 0),
        0
      ) || 0;
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [periodData, cumulativeData, accumulativeData, modalCompanies] =
        await Promise.all([
      ...
      setTotalPembelianGabungan(
        periodData.totalPembelianReady + periodData.totalBookedHargaBeli
      );
      ...
      const modalKalkulasi =
        periodData.totalBooked +
        ...
        <Card>
          <CardContent className="p-6">
            ...
            <p className="text-2xl font-bold text-indigo-600">
              {formatCurrency(totalModalCompanies)}
            </p>
          </CardContent>
        </Card>
      </div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between items-center p-2 bg-white rounded border">
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        setShowCompanyBreakdown(!showCompanyBreakdown)
                      }
                ...
                <div className="flex justify-between items-center p-2 bg-white rounded border">
                  ...
                  <span className="font-medium">
                    {formatCurrency(totalPembelianGabungan)}
                  </span>
                </div>
                <div className="flex justify-between items-center p-2 bg-white rounded border">
                ...
                <div className="flex justify-between items-center p-2 bg-white rounded border">
                  ...
                  <span className="font-medium">
                    {formatCurrency(totalKeuntunganBiroJasa)}
                  </span>
                </div>
                <div className="flex justify-between items-center p-2 bg-white rounded border">
              ...
              <div className="mt-4 w-full">
                ...
                <div
                  className={`h-2 rounded-full ${
                    cashFlowOperasional >= totalModalCompanies * 0.2
                      ? "bg-green-500"
                      : cashFlowOperasional >= 0
                      ? "bg-yellow-500"
                      : "bg-red-500"
                ...
                {keuntunganData.length === 0 ? (
                  ...
                  keuntunganData.map((item, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell>
                        {item.modal > 0
                          ? ((item.profit / item.modal) * 100).toFixed(2)
                          : "0.00"}
                        %
                      </TableCell>
                      <TableCell>
                        {new Date(item.tanggal_jual).toLocaleDateString(
                          "id-ID"
                        )}
                      </TableCell>
                    </TableRow>
